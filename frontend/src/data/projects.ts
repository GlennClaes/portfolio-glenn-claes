...
type BaseProject = Omit<Project, keyof ProjectText>;
...
export function getProjects(locale: Locale): Project[] {
  const texts = projectTexts[locale];
  return (Object.keys(baseProjects) as ProjectId[]).map((id) => {
    const base = baseProjects[id];
    const text = texts[id];
    return {
      ...base,
      title: text.title,
      ...
      cta: { ...base.cta, label: text.ctaLabel },
    };
  });
}
